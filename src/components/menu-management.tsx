...
export default function MenuManagement() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "error":
        return (
          <Badge className="bg-[#fe0000] hover:bg-red-700 text-white">
            Error
          </Badge>
        );
      default:
        return (
          <Badge className="bg-gray-200 text-[#040919] border border-gray-300">
            Unknown
          </Badge>
        );
    }
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "warning":
        return (
          <AlertTriangle className="h-4 w-4 sm:h-5 sm:w-5 text-[#fece00]" />
        );
      case "error":
  ...
  return (
    <div className="space-y-4 sm:space-y-6 lg:space-y-8">
      ...
      <Card className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 border border-gray-200">
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl lg:text-2xl font-['Playfair_Display'] text-[#040919]">
            Google Sheet Menu Integration
          </CardTitle>
          <CardDescription className="text-sm sm:text-base text-[#696868]">
            Your menu is managed directly through your linked Google Sheet.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 sm:space-y-6">
            ...
            <div className="flex items-start lg:items-center space-x-3">
              ...
              <div className="space-y-1">
                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                  <p className="font-medium text-sm sm:text-base text-[#040919]">
                    Integration Status:
                  </p>
                  {getStatusBadge(googleSheetStatus.status)}
                </div>
                <p className="text-xs sm:text-sm text-[#696868]">
                  Last Synced: {googleSheetStatus.lastSync}
                </p>
                {googleSheetStatus.errorMessage && (
                  <p className="text-xs sm:text-sm text-[#fe0000]">
                    {googleSheetStatus.errorMessage}
                  </p>
                )}
              </div>
          ...
          <div className="space-y-3 sm:space-y-4">
            <h3 className="font-semibold text-base sm:text-lg lg:text-xl text-[#040919]">
              How it works:
            </h3>
            <ul className="list-disc list-inside text-[#696868] space-y-2 sm:space-y-3 text-sm sm:text-base">
              <li>
                Your menu categories, items, and prices are pulled directly from
                your Google Sheet.
              </li>
              <li>
                Any changes you make in the Google Sheet will reflect on your
                digital menu in real-time (or after a short sync).
              </li>
              <li>
                Ensure your sheet follows the specified format for proper
                display.
              </li>
            </ul>
          </div>
